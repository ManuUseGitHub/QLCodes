#!/bin/bash
. ./shell/func.sh

OUT_FILE="./dist/qlCodes.json"

start_time=$(date +%s)

DEBUG=false
PRODUCTION=false
MAKE_KEYS=false

for arg in "$@"; do
  case $arg in
    --debug)
      DEBUG=true
      ;;
    --make-keys)
      MAKE_KEYS=true
      ;;
    --prod)
      PRODUCTION=true
      ;;
  esac
done

export DEBUG
export MAKE_KEYS
export PRODUCTION

clear
print_box "$(basename "$0") execution" 64

echo -e "MODE"
echo -e "${YELLOW} $@ ${NC}"

if ! [ -d "$DIR/resources" ]; then
  printStep "computing resources from archive"
  ./shell/settle.sh
fi

if ! [ -d "$DIR/Debug" ]; then
  ./shell/settle.sh
  mkdir ./resources/Debug
fi

echo -e "reference files read from\n${YELLOW}$DIR/resources/CSV${NC}\n"

printStep "Composing the resulting JSON file at\n${YELLOW}$DIR${NC}\n"
printStep "Composing ${YELLOW}(...)/$OUT_FILE${NC}"

node ./src/composeCsv.mjs "$DIR/resources/CSV/POSTGRES_states2.csv" --flags 'pgsql:cockroach' |\
node ./src/composeCsv.mjs "$DIR/resources/CSV/IBM_states2.csv"      --flags 'ibm' |\
node ./src/composeCsv.mjs "$DIR/resources/CSV/oracle_states.csv"    --flags 'oracle' |\
node ./src/composeCsv.mjs "$DIR/resources/CSV/MARIA_DB_codes2.csv"  --flags 'maria' |\
node ./src/composeCsv.mjs "$DIR/resources/CSV/sap_hana_states.csv"  --flags 'sap_hana' |\
node ./src/composeCsv.mjs "$DIR/resources/CSV/gSpanner_states.csv"  --flags 'google_spanner' |\
node ./src/pretty.mjs --file "$DIR/$OUT_FILE"
printSuccess composed

if ! $DEBUG
then
  print_box cleanup 64
  printStep "removing processing files."
  rm -rf -v $DIR/resources/Debug | ./shell/display.sh
  printSuccess removed
fi
if $PRODUCTION ;
then
  printStep "removing resources files."
  rm -rf -v $DIR/resources | ./shell/display.sh
  printSuccess removed
fi

end_time=$(date +%s)
duration=$((end_time - start_time))
echo -ne "\nExecution complete in ${YELLOW}${duration}s${NC}$(tput el)\n"

print_box finish 64
